...
import com.adnonstop.library.ViewCreateUtil;
...
public abstract class BaseAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case TYPE_RELOAD_VIEW:
                baseViewHolder = BaseViewHolder.create(mReloadView);
                break;
        }
    ...
    /**
     * LayoutManager
     * >StaggeredGridLayoutManager  staggered[ˈstægəd]
     * >LinearLayoutManager
     * > >GridLayoutManager
     * <p>
     ...
     * called by the recyclerView when it starts observing this adapter.
     * this adapter is BaseAdapter.calss
     *
     * @param recyclerView
     */
    @Override
    public void onAttachedToRecyclerView(RecyclerView recyclerView) {
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ...
            gridManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {//to query each item occupy's span
                @Override
                public int getSpanSize(int position) {
                    if (isFooterView(position)) {
                        return gridManager.getSpanCount();//footerView occupy a line
                    }
                    return 1;
                }
            });
        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    Log.i("loadMore", "onScrolled:  = stateChanged");
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                Log.i("loadMore", "onScrolled:  = onScrolled");
                //加载更多的操作应该在onScrollStateChanged里做的；这里做是为了实现自动加载，填满列表；
                if (isAutoLoadMore && findLastVisibleItemPosition(layoutManager) + 1 == getItemCount()) {
                    scrollLoadMore();
                    Log.i("loadMore", "onScrolled:  = loadMore = true");
                } else if (isAutoLoadMore) {
                    isAutoLoadMore = false;
                    Log.i("loadMore", "onScrolled:  = loadMore = false");
                }
            }
        });
    }
    ...
    private int findLastVisibleItemPosition(RecyclerView.LayoutManager layoutManager) {
        if (layoutManager instanceof LinearLayoutManager) {
            ...
            return ViewCreateUtil.findMax(lastVisibleItemPositions);
        }
        return -1;
    }
    ...
    public void setLoadingView(int loadingId) {
        setLoadingView(ViewCreateUtil.inflate(mContext, loadingId));
    }
    ...
    public void setLoadFailedView(int loadFailedId) {
        setLoadFailedView(ViewCreateUtil.inflate(mContext, loadFailedId));
    }
    ...
    public void setLoadEndView(int loadEndId) {
        setLoadEndView(ViewCreateUtil.inflate(mContext, loadEndId));
    }
